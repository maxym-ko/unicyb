...
sf::RenderWindow *Game::window = new sf::RenderWindow(sf::VideoMode(640, 480), "Snake",
                                                      sf::Style::Titlebar | sf::Style::Close);
int Game::score = 0;
bool Game::alive = true;

const int Game::range_x_from = 0;
const int Game::range_x_to = 39;
const int Game::range_y_from = 0;
const int Game::range_y_to = 29;
...
std::uniform_int_distribution<int> Game::distribution_x(Game::range_x_from, Game::range_x_to);
std::uniform_int_distribution<int> Game::distribution_y(Game::range_y_from, Game::range_y_to);
...
std::pair<int, int> Game::getNewFoodLocation() {
    ...
    do {
        temporalLocation = std::pair<int, int>(Game::distribution_x(Game::generatorX),
                                               Game::distribution_y(Game::generatorY));
    } while (collides(temporalLocation, Game::snake_body));

    return temporalLocation;
}

bool Game::collides(std::pair<int, int> coordinate, const std::vector<std::pair<int, int>> &body_coords) {
    for (auto &coord : body_coords) {
        if (coordinate.first == coord.first && coordinate.second == coord.second) return true;
    }
...
void Game::moveSnake() {
    ...
    switch (new_direction) {
        case Direction::UP:
            if (snake_body[0].second - 1 >= range_y_from)
                new_head_pos.second = snake_body[0].second - 1;
            else
                new_head_pos.second = range_y_to;
            new_head_pos.first = snake_body[0].first;
            break;

        case Direction::DOWN:
            if (snake_body[0].second + 1 <= range_y_to)
                new_head_pos.second = snake_body[0].second + 1;
            else
                new_head_pos.second = range_y_from;
            new_head_pos.first = snake_body[0].first;
            break;

        case Direction::LEFT:
            if (snake_body[0].first - 1 >= range_x_from)
                new_head_pos.first = snake_body[0].first - 1;
            else
                new_head_pos.first = range_x_to;
            new_head_pos.second = snake_body[0].second;
            break;

        case Direction::RIGHT:
            if (snake_body[0].first + 1 <= range_x_to)
                new_head_pos.first = snake_body[0].first + 1;
            else
                new_head_pos.first = range_x_from;
            new_head_pos.second = snake_body[0].second;
            break;
    }
